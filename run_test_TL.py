...
all_states = trajs_train[
    ['state_done', 'cumulative_overdue_early_difference', 'gender', 'amount', 'num_loan', 'duration', 'year_ratio',
     'diff_city', 'marriage', 'kids', 'month_in', 'housing', 'edu', 'motivation']].drop_duplicates().values.tolist()
...
all_states1 = trajs1_pd_train[
    ['state_done', 'cumulative_overdue_early_difference', 'gender', 'amount', 'num_loan', 'duration', 'year_ratio',
     'diff_city', 'marriage', 'kids', 'month_in', 'housing', 'edu', 'motivation']].drop_duplicates().values.tolist()
all_states2 = trajs2_pd_train[
    ['state_done', 'cumulative_overdue_early_difference', 'gender', 'amount', 'num_loan', 'duration', 'year_ratio',
     'diff_city', 'marriage', 'kids', 'month_in', 'housing', 'edu', 'motivation']].drop_duplicates().values.tolist()
...
for loan_id in loan_ids1:
    ...
    states = df1[
        ['state_done', 'cumulative_overdue_early_difference', 'gender', 'amount', 'num_loan', 'duration', 'year_ratio',
         'diff_city', 'marriage', 'kids', 'month_in', 'housing', 'edu', 'motivation']].values.tolist()
    states.insert(0, states[0])
    states_index = []
    for s in states:
...
nS1 = trajs1_pd_train[
    ['state_done', 'cumulative_overdue_early_difference', 'gender', 'amount', 'num_loan', 'duration', 'year_ratio',
     'diff_city', 'marriage', 'kids', 'month_in', 'housing', 'edu', 'motivation']].drop_duplicates().shape[0]
...
df1_final = trajs1_pd_train[
    ['state_done', 'cumulative_overdue_early_difference', 'gender', 'amount', 'num_loan', 'duration', 'year_ratio',
     'diff_city', 'marriage', 'kids', 'month_in', 'housing', 'edu', 'motivation']].drop_duplicates()
...
trajs1_pd_test = trajs1_pd_test.merge(df1_final,
                                      on=['state_done', 'cumulative_overdue_early_difference', 'gender', 'amount',
                                          'num_loan', 'duration', 'year_ratio', 'diff_city', 'marriage', 'kids',
                                          'month_in', 'housing', 'edu', 'motivation'], how='left')
...
# estimate reward for q learning
print('######################### estimate reward for q learning ###############################################')
state_action_pairs = trajs1_pd_test[
    ['gender', 'amount', 'num_loan', 'duration', 'year_ratio', 'diff_city', 'marriage', 'kids', 'month_in', 'housing',
     'edu', 'motivation', 'state_done', 'cumulative_overdue_early_difference', 'opt_action_qlearning']].values
rewards = trajs1_pd_test['reward'].values
...
state_action_pairs = trajs1_pd_test[
    ['gender', 'amount', 'num_loan', 'duration', 'year_ratio', 'diff_city', 'marriage', 'kids', 'month_in', 'housing',
     'edu', 'motivation', 'state_done', 'cumulative_overdue_early_difference', 'action_num']].values
rewards = trajs1_pd_test['reward'].values
...
# %%
########################################################### Rule learning ##########################################################################
rule_states = trajs1_pd_test[
    ['gender', 'amount', 'num_loan', 'duration', 'year_ratio', 'diff_city', 'marriage', 'kids', 'month_in', 'housing',
     'edu', 'motivation', 'state_done', 'cumulative_overdue_early_difference']]
...
state_list = ['gender', 'amount', 'num_loan', 'duration', 'year_ratio', 'diff_city', 'marriage', 'kids', 'month_in',
              'housing', 'edu', 'motivation']
...
qlearning2_transfer, qlearning2_reward_transfer = qlearning(trajs2, nS2, nA2, recommended_Q=trajs2_t_list, alpha=0.4,
                                                            gamma=gamma, epsilon=0.9)
...
trajs2_pd_test_copy = trajs2_pd_test.merge(df2_final,
                                           on=['state_done', 'cumulative_overdue_early_difference', 'gender', 'amount',
                                               'num_loan', 'duration', 'year_ratio', 'diff_city', 'marriage', 'kids',
                                               'month_in', 'housing', 'edu', 'motivation'], how='left')
...
# estimate reward for q learning
print('######################### estimate reward for q learning ###############################################')
state_action_pairs = trajs2_pd_test_copy[
    ['gender', 'amount', 'num_loan', 'duration', 'year_ratio', 'diff_city', 'marriage', 'kids', 'month_in', 'housing',
     'edu', 'motivation', 'state_done', 'cumulative_overdue_early_difference', 'opt_action_qlearning']].values
state_action_pairs_tf = trajs2_pd_test_copy[
    ['gender', 'amount', 'num_loan', 'duration', 'year_ratio', 'diff_city', 'marriage', 'kids', 'month_in', 'housing',
     'edu', 'motivation', 'state_done', 'cumulative_overdue_early_difference', 'opt_action_qlearning_tf']].values
...
model = cnn_model_fn()
state_action_pairs = np.reshape(state_action_pairs, (state_action_pairs.shape[0], state_action_pairs.shape[1], 1))
state_action_pairs_tf = np.reshape(state_action_pairs_tf,
                                   (state_action_pairs_tf.shape[0], state_action_pairs_tf.shape[1], 1))
model = load_model('./' + '0-reg-0.0594.hdf5')
# no batch if predicting a single data
pred = model.predict(state_action_pairs, batch_size=64)
